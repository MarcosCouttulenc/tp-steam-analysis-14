...
class GameWorker:
    ...
    ## --------------------------------------------------------------------------------      
    def process_control_master_eof_handler(self, socket_master_slave, socket_master_slave_addr, barrier: threading.Barrier):
        #Tiene que guardar los estados en disco. La lista de finished client
    ...
    def process_message_slave_eof(self, ch, method, properties, message: Message):
        ...
        while True:
            ...
            # Si el MASTER se cayo en el momento en el que el master recibe
            if sended == None:
